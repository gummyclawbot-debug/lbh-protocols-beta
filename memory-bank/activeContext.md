...
- User clarified that patient range safeguards must be advisory rather than hard blocking. Age/Height/Weight/SCr now accept unrestricted numeric entry and continue calculations while out-of-range values retain the red, accessible field warnings; missing values still suppress derived output. Native `min`/`max` constraints were removed.
